...
# Define variables for reports outside of conditional blocks
diab_diagnosis = ''
heart_diagnosis = ''
parkinsons_diagnosis = ''
    
def send_to_doctor(data):
    url = "http://localhost:4000/api/v1/reports/generateReportAndSave"
    headers = {
...
if selected == 'Diabetes Prediction':
    ...
    if st.button('Diabetes Test Result'):
        ...
        #Prepare data to send to the doctor
        report = ask_order(diab_diagnosis, user_diab_prompt)
        data_to_send = {
                "email": email,
                "password": password,
...
if selected == 'Heart Disease Prediction':
    ...
    if st.button('Heart Disease Test Result'):
        ...
        report = ask_order(heart_diagnosis,User_Heart_prompt)
        data_to_send = {
                "email": email,
                "password": password,
                "age": age,
...
if selected == "Parkinsons Prediction":
    ...
    if st.button("Parkinson's Test Result"):
        ...
        report = ask_order(parkinsons_diagnosis,Parkinsons_prompt)
        data_to_send = {
                "email": email,
                "password": password,
                "MDVP:Fo(Hz)":fo,
